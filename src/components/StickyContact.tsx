...
const StickyContact = () => {
  const [settings, setSettings] = useState<{ phone?: string } | null>(null);
  const [isVisible, setIsVisible] = useState(false);

  useEffect(() => {
    // Biraz scroll sonra göster
    ...
    // Sayfa yüklendiğinde hemen göster
    setIsVisible(true);
  ...
  return (
    ...
    >
      {/* WhatsApp Button */}
      <a
        href={`https://wa.me/${whatsappNumber}?text=Merhaba,%20ürünleriniz%20hakkında%20bilgi%20almak%20istiyorum.`}
        target="_blank"
      ...
      <a
        ...
        className="group relative flex items-center justify-center w-14 h-14 bg-gradient-to-br from-[#8B7355] to-[#A0826D] hover:from-[#A0826D] hover:to-[#8B7355] rounded-full shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-110"
        title="Hemen ara"
      >
        {/* Phone Icon */}
        ...
        {/* Pulse Animation */}
        <span className="absolute inset-0 rounded-full bg-[#8B7355] animate-pulse opacity-20"></span>
      </a>
    </div>
  );
};
